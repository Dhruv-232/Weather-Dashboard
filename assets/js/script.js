...
function addToSearchHistory(cityName) {
    // remove any existing entries with the same city name
    $('.past-search:contains("' + cityName + '")').remove();

    // create new search history entry and container
    var searchHistoryEntry = $("<p>").addClass("past-search").text(cityName);
    var searchEntryContainer = $("<div>").addClass("past-search-container").append(searchHistoryEntry);

    // append entry container to search history container
    var searchHistoryContainerEl = $("#search-history-container");
    searchHistoryContainerEl.append(searchEntryContainer);

    // update saved searches array with previously saved searches
    var savedSearches = [];
...
var loadSearchHistory = function() {
    ...
    // Convert the saved search history string into an array.
    savedSearchHistory = JSON.parse(savedSearchHistory);

    // Iterate through the savedSearchHistory array and create an entry for each item in the list.
    for (var i = 0; i < savedSearchHistory.length; i++) {
        createSearchHistoryEntry(savedSearchHistory[i]);
    }
};
